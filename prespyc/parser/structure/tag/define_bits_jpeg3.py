...
class DefineBitsJPEG3Tag:
    """
    A self-contained image character with an optional alpha channel. Implements `DefineBitsJPEGTag`.
    ...
    def read(cls, reader: Reader, end: int) -> Self:
        """Read a DefineBitsJPEG3 tag, whose data ends at the `end` byte offset."""
        character_id = reader.read_ui16()
        image_data = reader.read_bytes(reader.read_ui32())
        alpha_data = None
